...
describe('generated brand documents', () => {
  ...
  it('keeps robots, the sitemap, and llms.txt on the brand domain only', () => {
    for (const brand of all) {
      ...
      expect(documents.llmsText).toContain(`https://${brand.host}/withdraw`)
...
describe('privacy notice', () => {
  it('names the controller, the basis, retention, rights, and the authority, and nothing beyond the domain', () => {
    for (const brand of all) {
      ...
      expect(text).toContain(`${controller.retention.subscriptionMonths} months`)
      expect(text).toContain(controller.authority.name)
      expect(text).toContain('Withdraw page')
      expect(text).toContain('one calendar month')
      expect(text).toContain(`${controller.retention.requestDays} days`)
      expect(text).toContain('does not call AI services')
